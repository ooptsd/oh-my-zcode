...
Set up omz notification integrations so you're alerted when sessions end, need input, or complete background tasks.
...
Set up Telegram notifications so omz can message you when sessions end, need input, or complete background tasks.
...
2. Send /newbot
3. Choose a name (e.g., "My omz Notifier")
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=omz test notification - Telegram is configured!")
...
To reconfigure: /omz:configure-notifications telegram
To configure Discord: /omz:configure-notifications discord
To configure Slack: /omz:configure-notifications slack
```
...
Set up Discord notifications so omz can ping you when sessions end, need input, or complete background tasks.
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}omz test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
To reconfigure: /omz:configure-notifications discord
To configure Telegram: /omz:configure-notifications telegram
To configure Slack: /omz:configure-notifications slack
```
...
Set up Slack notifications so omz can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "omz Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}omz test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
To reconfigure: /omz:configure-notifications slack
To configure Discord: /omz:configure-notifications discord
To configure Telegram: /omz:configure-notifications telegram
```
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active omz mode name
...
- `/omz:configure-openclaw` — Configure OpenClaw gateway integration
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"omz test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
