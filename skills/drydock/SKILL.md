...
| Pillar | Surfaces |
|---|---|
| Context (shared background) | `CONTEXT.md` (glossary) + `docs/business/` + `docs/adr/` + omz wiki |
...
- omz installed? — only worth checking when running inside an omz session; outside one, skip this check silently (the harness works with or without omz)
...
```json
{
  ...
  "stableTokens": [
    "CONTEXT.md", "documentLanguage", "/omz:launch", "--serial",
...
## Agent guide
- Delivery follows the canonical workflow plan → execute → review → verify; `/omz:launch` is an optional governed delivery pipeline (opt-in, invoke explicitly)
...
## Agent 指南
- 交付遵循 canonical 工作流 plan → execute → review → verify；`/omz:launch` 是可选的受治理交付管道（opt-in，需要时显式调用）
...
## Agent 指南
- 交付遵循 canonical 工作流 plan → execute → review → verify；`/omz:launch` 是可選的治理交付管道（opt-in，必須明確呼叫）
...
- **wiki** (omz) compounds session knowledge; promote anything referenced twice into docs/business/
