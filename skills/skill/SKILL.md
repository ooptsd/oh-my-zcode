...
Meta-skill for managing omz skills via CLI-like commands.
...
```
BUILT-IN SKILLS (bundled with omz):
...
**Built-in skill note:** Built-in skills are bundled with omz and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
- `/omz:skill list` - Show detailed skill inventory
- `/omz:skill add` - Start skill creation (invoke skillify)
- `/omz:skill scan` - Scan both skill directories
...
- `/omz:skillify` - Extract a skill from current conversation (`/omz:learner` is a deprecated alias)
- `/omz:note` - Save quick notes (less formal than skills)
- `/omz:deepinit` - Generate AGENTS.md codebase hierarchy
...
- Run `/omz:skill list` periodically to review your skill library
