...
You are the **loop controller** for the self-improvement system. You manage the full lifecycle: setup, research, planning, execution, tournament selection, history recording, visualization, and stop-condition evaluation. You delegate to specialized omz agents and coordinate their inputs and outputs.
...
omz mode lifecycle: `.omc/state/sessions/{sessionId}/self-improve-state.json`
...
| Step | Role | omz Agent | Model |
|------|------|-----------|-------|
| Research | Codebase analysis + hypothesis generation | general-purpose Agent | opus |
| Planning | Hypothesis → structured plan | omz:planner | opus |
| Architecture Review | 6-point plan review | omz:architect | opus |
| Critic Review | Harness rule enforcement | omz:critic | opus |
| Execution | Implement plan + run benchmark | omz:executor | opus |
| Git Operations | Atomic merge/tag/PR | omz:git-master | sonnet |
...
All git operations happen inside the target repo, NOT in the omz project root.
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `omz:git-master`:
  ```
...
Spawn N `omz:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `omz:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `omz:critic` with the plan + harness rules:
...
For each approved plan, spawn `omz:executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `omz:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
4. Print summary report:
   ...
   ```
5. Run `/omz:cancel` for clean state cleanup
