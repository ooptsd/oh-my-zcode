...
# Mixed: Codex for backend analysis, Gemini/Antigravity for frontend (delegate per-task with /omz:ask)
```
...
User: "/team 3:executor fix all TypeScript errors"
      ...
      [TEAM ORCHESTRATOR (Lead)]
              ...
              +-- Completion
                      ...
                      -> clear omz team state (no TeamDelete call)
...
- **Cancel:** `/omz:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.omc/handoffs/` are preserved for potential resume.
...
- Extract **agent-type**, validate it maps to a known omz subagent
...
Derive a slug such as `fix-ts-errors` for omz state, prompt labels, handoffs, and human-readable reporting only. Native Claude Code may accept `team_name` as legacy metadata, but it is ignored for routing.

Write omz state using the `state_write` MCP tool for proper session-scoped persistence:
...
| `team_name`      | string  | omz slug for state, handoffs, and reporting; ignored by native Claude Code routing |
...
On every stage transition, update omz state:
...
4. **Clean up native team state** -- Claude Code 2.1.178+ has no `TeamDelete`; after teammates acknowledge shutdown, clear omz state and any local task bookkeeping.
5. **Clean omz state** -- Remove `.omc/state/team-state.json`
...
```
You are a TEAM WORKER in omz team "{team_name}". Your name is "{worker_name}".
...
**CRITICAL: Steps must execute in exact order. Never clear omz team state before shutdown is confirmed or timed out.**
...
After approval, the teammate terminates or stops accepting new work. Claude Code 2.1.178+ does not expose per-team config membership or TeamDelete cleanup; track acknowledgements in omz state/reporting instead.

**Step 4: Clear omz team state — only after ALL teammates confirmed or timed out**

Claude Code 2.1.178+ has no `TeamDelete`. Clear omz team state and local task bookkeeping after the blocking shutdown pass completes.

**Step 5: Orphan scan for omz tmux/CLI workers only**

For legacy omz tmux/CLI worker runs (`omc team` / `/omc-teams`), check for worker processes that survived cleanup:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name fix-ts-errors
```

This scans for omz worker processes matching the team name and terminates stale orphans (SIGTERM → 5s wait → SIGKILL). Supports `--dry-run` for inspection.

**Shutdown sequence is BLOCKING:** Do not clear omz team state until all teammates have either:
...
1. Spawn `Task(subagent_type="omz:planner", ...)` with task description + codebase context
...
1. User invokes `/team ralph "task"` or `/omz:team ralph "task"`
...
4. If architect approves: both modes complete, run `/omz:cancel`
...
If the lead crashes mid-run, the team skill should detect existing omz state and resume:

1. Read `state_read(mode="team")` for the active omz team slug, phase, and worker labels
2. Use omz handoffs and task-list/TodoWrite state to determine current progress
...
| ----------------------- | ---------------------------------------------------------------- | -------------------------------------- |
| **Storage**             | omz state/handoffs plus Claude Code's current task-list surface   | SQLite in `.omc/state/swarm.db`        |
...
| **Shutdown**            | Graceful request/response protocol plus omz state clear           | Signal-based termination               |
| **Agent lifecycle**     | Tracked by named Agent/Task spawns and omz state                  | Manual tracking via heartbeat table    |
...
| **State cleanup**       | Clear omz team state after teammate shutdown                      | Manual `rm` of SQLite database         |
...
The `/omz:cancel` skill handles team cleanup:
...
If teammates are unresponsive, record the timeout, avoid spawning more work, and clear omz state only after the shutdown wait completes or the user force-cancels.
...
> **Note:** Native Claude Code teammates do not have a hardcoded model default; each teammate inherits the lead session's configured model unless overridden per role. omz's legacy CLI workers are separate processes: their provider and model come from omz's team routing, not native teammate inheritance.
>
> **Nesting (Claude Code 2.1.217+):** Claude teammates are full, independent Claude Code sessions in the session's implicit team (see Phase 5). Do not assume a teammate can or cannot spawn nested subagents: Claude Code 2.1.217–2.1.218 defaulted `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` to 1, while 2.1.219+ defaults to 3, and the current setting controls subagent layers below the main conversation. Plan each teammate as the unit that does the work unless nested delegation is deliberate and supported by the active runtime. omz CLI workers (`claude`, `codex`, `gemini`, `grok`, `cursor`, and `antigravity` via `ops.defaultAgentType`) are separate processes, not Claude Code subagents, and are unaffected by Claude's subagent depth setting.
>
> **Concurrency:** The current omz team CLI caps worker fan-out at 20 (`MAX_WORKER_COUNT`); although `ops.maxAgents` exists in the config schema, the current launcher does not consult it. Claude Code's `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` also defaults to 20, but that limit applies to Agent-spawned subagents; Claude Code documents agent-team teammates as following their own limits. Do not treat the two values as a shared cap or assume a full-size omz team leaves exactly zero headroom for ordinary subagents; size omz teams conservatively when the lead also needs Agent-spawned work.
...
1. Native Claude Code 2.1.178+ has no per-team `TeamDelete` cleanup. After shutdown is confirmed or timed out, clear omz state via MCP tools:
   ```
   ...
   ```
2. For legacy omz tmux/CLI workers, run the documented `omc team shutdown` / cleanup path.
3. Or run `/omz:cancel` which handles omz state cleanup automatically.

**IMPORTANT:** Clear omz team state only AFTER all teammates have been shut down or timed out.
...
4. **No TeamDelete cleanup** -- Claude Code 2.1.178+ removed `TeamDelete`; use shutdown messages plus omz state cleanup.
...
7. **Shutdown acknowledgements are state/reporting events** -- After a teammate approves shutdown and terminates, track that acknowledgement in omz state/reporting. Do not expect a Claude Code team membership config to update.
