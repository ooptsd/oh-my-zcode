---
name: omz-setup
description: Install or refresh omz for plugin, npm, and local-dev setups from the canonical setup flow
level: 2
---
...
Choose this setup flow when the user wants to **install, refresh, or repair omz itself**.
...
```
omz Setup - Configure omz

USAGE:
  /omz:omz-setup           Run initial setup wizard (or update if already configured)
  /omz:omz-setup --local   Configure local project (.claude/CLAUDE.md)
  /omz:omz-setup --global  Configure global settings (~/.claude/CLAUDE.md)
  /omz:omz-setup --force   Force full setup wizard even if already configured
  /omz:omz-setup --help    Show this help
...
MODES:
  ...
  Local Configuration (--local)
    ...
    - Use this to update project config after omz upgrades

  Global Configuration (--global)
    - Invokes the plugin-local coordinator through `scripts/setup-claude-md.sh`; the shell validates the coordinator response and its exit status before any post-install work
    - Reports coordinator-created byte-identical backups only for changed global files
    - Default: explicitly overwrites ~/.claude/CLAUDE.md so plain `claude` also uses omz
    - Optional preserve mode keeps the user's base `CLAUDE.md` and installs omz into `CLAUDE-omc.md` for `omc` launches
    - Applies to all Claude Code sessions
    - Preserves same-named legacy hook files unless their exact historical contents are independently verified
    - Use this to update global config after omz upgrades
...
EXAMPLES:
  /omz:omz-setup           # First time setup (or update CLAUDE.md if configured)
  /omz:omz-setup --local   # Update this project
  /omz:omz-setup --global  # Update all projects
  /omz:omz-setup --force   # Re-run full setup wizard
...
if [ -f "$CONFIG_FILE" ]; then
  if ! command -v jq >/dev/null 2>&1; then
    echo "ERROR: jq is required to inspect existing omz setup state. Existing config was not modified."
    exit 1
  fi
  if ! SETUP_COMPLETED=$(jq -r '.setupCompleted // empty' "$CONFIG_FILE" 2>/dev/null) \
    ...
    echo "ERROR: Existing omz config is invalid JSON. Existing config was not modified."
    exit 1
  fi

  if [ -n "$SETUP_COMPLETED" ] && [ "$SETUP_COMPLETED" != "null" ]; then
    echo "omz setup was already completed on: $SETUP_COMPLETED"
...
**Question:** "omz is already configured. What would you like to do?"
...
Read the file at `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omz-setup/phases/01-install-claude-md.md` and follow its instructions.
...
1. **Phase 1 - Install CLAUDE.md**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omz-setup/phases/01-install-claude-md.md` and follow its instructions.

2. **Phase 2 - Environment Configuration**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omz-setup/phases/02-configure.md` and follow its instructions. Phase 2 must delegate HUD/statusLine setup to the `hud` skill; do not generate or patch `statusLine` paths inline here.

3. **Phase 3 - Integration Setup**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omz-setup/phases/03-integrations.md` and follow its instructions.

4. **Phase 4 - Completion**: Read `${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/omz-setup/phases/04-welcome.md` and follow its instructions.
...
**Automatic**: Just run `/omz:omz-setup` - it will detect you've already configured and offer a quick "Update CLAUDE.md and clear retired setup values" option that skips the rest of the wizard.
...
**Manual options**:
- `/omz:omz-setup --local` to update project config only
- `/omz:omz-setup --global` to update global config only
- `/omz:omz-setup --force` to re-run the full wizard (reconfigure preferences)
