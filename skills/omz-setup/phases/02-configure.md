...
Ralph is an agent-driven persistence loop with no external language dependency. Its only hard prerequisites are the Node runtime that already runs omz's hooks and a writable config directory for `.omc/state/` state files. Verify both without blocking the rest of setup:

```bash
node --version || echo "ERROR: Node is required for omz hooks and Ralph state persistence."
...
After a marketplace update, Claude Code may still have old omz cache paths in the running session or plugin registry. Repair those references before any cache cleanup so setup does not repeatedly emit stale plugin directory errors.
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin omz"
  else
...
**Note:** Never write a new `defaultExecutionMode` value. Generic keywords no longer route through a configured execution mode; invoke `/omz:execute` or `/omz:team` directly instead.
...
## Step 2.5: Install omz CLI Tool

The omz CLI (`omc` command) provides standalone helper commands such as `omc hud`, `omc teleport`, and `omc team ...`.
...
if command -v omc &>/dev/null; then
  ...
  echo "omz CLI already installed: $OMC_CLI_VERSION"
...
**Question:** "Would you like to install the omz CLI globally for standalone helper commands? (`omc`, `omc hud`, `omc teleport`)"
...
if ! command -v npm &>/dev/null; then
  echo "WARNING: npm not found. Cannot install omz CLI automatically."
  ...
  if npm install -g oh-my-claude-sisyphus 2>&1; then
    echo "omz CLI installed successfully."
    if command -v omc &>/dev/null; then
      OMC_CLI_VERSION=$(omc --version 2>/dev/null | head -1 || echo "installed")
    ...
    echo "WARNING: Failed to install omz CLI (permission issue or network error)."
