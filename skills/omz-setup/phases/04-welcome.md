...
| cancelomc | Stop active omz modes | "cancelomc" |

CANONICAL WORKFLOWS (Tier-0):
omc-plan -> execute -> omc-review -> verify, invoked as /omz:omc-plan and /omz:omc-review.
...
- /omz:team 3:executor "fix all TypeScript errors"
- /omz:team 5:debugger "fix build errors in src/"
...
MCP SERVERS:
Register extra MCP servers (web search, GitHub, etc.) through Claude Code's native MCP config (`claude mcp add ...` or the path selected by `CLAUDE_MCP_CONFIG_PATH`; by default, the sibling `.claude.json` next to `${CLAUDE_CONFIG_DIR:-$HOME/.claude}`). omz's bundled MCP server is already registered via the plugin's .mcp.json.

HUD STATUSLINE:
The status bar now shows omz state. Restart Claude Code to see it.

omz CLI HELPERS (if installed):
...
/omz:omz-setup for setup, and Claude Code's native MCP config for mcp-setup;
/wiki for omc-reference).

### For Users Upgrading from 2.x (IS_UPGRADE is true):

```
omz Setup Complete! (Upgraded from 2.x)
...
omz includes rule templates you can copy to your project's `.claude/rules/` directory for automatic context injection:
...
Get the current omz version and mark setup complete:

```bash
# Get current omz version from CLAUDE.md
OMC_VERSION=""
CONFIG_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
case "$CONFIG_DIR" in
  "~") CONFIG_DIR="$HOME" ;;
