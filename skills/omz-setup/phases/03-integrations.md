...
grep -q "omz" "$CONFIG_DIR/settings.json" && echo "Plugin verified" || echo "Plugin NOT found - run: claude /install-plugin omz"
```

## Step 3.2: MCP Server Configuration (Pointer Only)

MCP servers extend Claude Code with additional tools (web search, GitHub, etc.). omz no longer ships an MCP setup skill; servers are registered through Claude Code's native MCP surfaces.
...
- omz keeps its own bundled MCP server (`bridge/mcp-server.cjs`) registered via `.mcp.json`; it requires no user action
...
Or by running `/omz:omz-setup --force` and choosing to enable teams.
