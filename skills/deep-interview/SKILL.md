...
If this raw bundled skill is loaded by Claude Code's native plugin skill loader through `/omz:deep-interview` or `Skill("omz:deep-interview")`, do not treat that path as permission to skip rendered omz setup. The user-facing preferred invocation is `/deep-interview`; do not recommend or advertise `/omz:deep-interview` as the deep-interview entrypoint. Regardless of invocation path, Phase 0 below remains blocking and must resolve `omc.deepInterview.ambiguityThreshold` from settings before any announcement, state write, question, or ambiguity score.
...
**Autoresearch override:** if `--autoresearch` is active, skip the standard execution options below. The only valid bridge is the `Skill("omz:autoresearch")` handoff described above. The `omc autoresearch` CLI is a hard-deprecated shim and must not be used for execution.
...
2. **Execute with autopilot**
   ...
   - Action: Invoke `Skill("omz:autopilot")` with the spec file path as context only after the user explicitly selects this execution option. The spec replaces autopilot's Phase 0 — autopilot starts at Phase 1 (Planning).
...
3. **Execute with ralph**
   ...
   - Action: Invoke `Skill("omz:ralph")` with the spec file path as the task definition.

4. **Execute with team**
   - Description: "N coordinated parallel agents — fastest execution for large specs"
   - Action: Invoke `Skill("omz:team")` with the spec file path as the shared plan.
...
- Preserve the AskUserQuestion path for omz-native interaction; do not introduce OMX-only structured-question transport into this skill
- Use `Task(subagent_type="omz:explore", model="haiku")` for brownfield codebase exploration (run BEFORE asking user about codebase)
...
/deep-interview "vague idea"
  ...
  → Skill("omz:plan") with --consensus --direct (spec as input, skip interview)
