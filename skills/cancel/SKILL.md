...
description: Cancel any active omz mode (autopilot, ralph, ultragoal, swarm, ultrapilot, pipeline, team) and clean up retired legacy state
...
Intelligent cancellation that detects and cancels the active omz mode.

**The cancel skill is the standard way to complete and exit any omz mode.**
...
- **Team**: Requests shutdown from all teammates through the active team/conversation surface, waits for responses/timeouts, clears omz team state, clears linked ralph if present. Claude Code 2.1.178+ has no TeamDelete.
...
```
ToolSearch(query="select:mcp__plugin_omz_t__state_clear,mcp__plugin_omz_t__state_read,mcp__plugin_omz_t__state_write,mcp__plugin_omz_t__state_list_active,mcp__plugin_omz_t__state_get_status")
```
...
`/omz:cancel` follows the session-aware state contract:
...
3. If a `session_id` was supplied to `/omz:cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omc/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
Teams are detected through omz team state, not removed Claude Code `~/.claude/teams` config directories:

```bash
# Check for active omz team state
...
```
For the active omz team state:
  1. Read team_name and worker labels from omz state/handoffs/task bookkeeping
...
```
  ...
  3. Run omz tmux/CLI orphan scan only for legacy `omc team` / `/omc-teams` workers (see below)
  4. Emit structured cancel report
```

**Orphan Detection (Post-Cleanup):**

For legacy omz tmux/CLI worker runs, verify no worker processes remain:
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name "{team_name}"
```

The orphan scanner:
1. Checks `ps aux` (Unix) or `tasklist` (Windows) for omz worker processes with `--team-name` matching the cleaned-up team
...
Team "{team_name}" cancelled:
  ...
  - omz state cleared: yes/no
  - Manual cleanup needed: yes/no
    Path: omz team state / tmux worker processes, if any
```

**Implementation note:** The cancel skill is executed by the LLM, not as a bash script. When you detect an active team:
1. Read `state_read(mode="team")` to find the active omz team
...
Report: "No active omz modes detected. Use --force to clear all state files anyway."
...
| Force | "All omz modes cleared. You are free to start fresh." |
| None | "No active omz modes detected." |

## What Gets Preserved

| Mode | State Preserved | Resume Command |
|------|-----------------|----------------|
| Autopilot | Yes (phase, files, spec, plan, verdicts) | `/omz:autopilot` |
| Ralph | No | N/A |
| Ultrawork | No | N/A |
