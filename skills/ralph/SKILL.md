...
7. **Reviewer verification** (tiered, against acceptance criteria):
   ...
   - **On APPROVAL: immediately proceed to Step 7.5 in the same turn. Do NOT pause to report the verdict to the user — reporting happens only at Step 8 (`/omz:cancel`) or on rejection (Step 9). Treating an approved verdict as a reporting checkpoint is a polite-stop anti-pattern.**

7.5 **Mandatory Deslop Pass** (runs unconditionally after Step 7 approval, unless `{{PROMPT}}` contains `--no-deslop`):

- **Invoke the `ai-slop-cleaner` skill via the Skill tool: `Skill("omz:ai-slop-cleaner")`** — it is a Skill, not an agent. If you mistakenly call it via `Task(subagent_type="omz:ai-slop-cleaner")`, omz's PreToolUse hook denies the call with the correct Skill-tool identifier; do not substitute a similarly-named agent. Run in standard mode (not `--review`) on the files changed during the current Ralph session only.
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/omz:cancel` to cleanly exit and clean up all state files
...
- Use `Task(subagent_type="omz:architect", ...)` for architect verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Use `Task(subagent_type="omz:critic", ...)` when `--critic=critic`
...
- **Skill vs agent invocation**: skills (e.g. `ai-slop-cleaner`) are invoked via the Skill tool: `Skill("omz:ai-slop-cleaner")`; agents (e.g. `architect`, `critic`, `executor`) via `Task(subagent_type="omz:<name>")`. omz's PreToolUse hook denies a Task/Agent call whose `subagent_type` names a bundled skill and returns the correct Skill-tool identifier — do not substitute a similarly-named agent as a "closest match".
  </Tool_Usage>
...
Task(subagent_type="omz:executor", model="haiku", prompt="Add type export for UserConfig")
Task(subagent_type="omz:executor", model="sonnet", prompt="Implement the caching layer for API responses")
Task(subagent_type="omz:executor", model="opus", prompt="Refactor auth module to support OAuth2 flow")
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/omz:cancel`
...
- [ ] `/omz:cancel` run for clean state cleanup
</Final_Checklist>
