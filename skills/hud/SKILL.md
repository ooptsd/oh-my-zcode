...
Configure the omz HUD (Heads-Up Display) for the statusline.
...
| Command | Description |
|---------|-------------|
| `/omz:hud` | Show current HUD status (auto-setup if needed) |
| `/omz:hud setup` | Install/repair HUD statusline |
| `/omz:hud minimal` | Switch to minimal display |
| `/omz:hud focused` | Switch to focused display (default) |
| `/omz:hud full` | Switch to full display |
| `/omz:hud status` | Show detailed HUD status |

## Auto-Setup

When you run `/omz:hud` or `/omz:hud setup`, the system will automatically:
...
```
[omz] branch:main | ralph:3/10 | US-002 | skill:planner | ctx:67% | agents:2 | bg:3/5 | todos:2/5
```

### Full
Shows everything including multi-line agent details:
```
[omz] repo:oh-my-claudecode branch:main | ralph:3/10 | US-002 (2/5) | ctx:[████░░]67% | agents:3 | bg:3/5 | todos:2/5
...
1. Run `/omz:hud setup` to auto-install and configure
2. Restart Claude Code after setup completes
3. If still not working, run `/omz:omz-doctor` for full diagnostics

**Legacy string format migration:** Older omz versions wrote `statusLine` as a plain string (e.g., `"~/.claude/hud/omc-hud.mjs"`). Modern Claude Code (v2.1+) requires an object format. Running the installer or `/omz:hud setup` will auto-migrate legacy strings to the correct object format:
```json
{
  "statusLine": {
    "type": "command",
