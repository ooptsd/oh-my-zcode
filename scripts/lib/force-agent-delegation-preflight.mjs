...
//             "denyMessage": "10+ raw Reads in 2 min — spawn Agent(subagent_type='omz:explore', model='haiku', ...). Bypass: ALLOW_RAW_READ=1.",
 ...
 *   Typically `<cwd>/.omc/state` to mirror the rest of omz state storage.
 * @param {object} [args.env=process.env] - Environment for bypass-flag lookup.
 * @param {Function} [args.loadOmcConfig] - Function returning the resolved omz
        ...
        return {
          ...
          reason:
            typeof rule.denyMessage === 'string' && rule.denyMessage
              ? rule.denyMessage
              : `[omz] Force-agent-delegation: ${observed} ${toolName} in last ${windowSeconds}s ` +
