...
/**
 * PreToolUse Hook: omz Reminder Enforcer (Node.js)
...
// OMC_MODEL_* is intentionally excluded: those are omz-internal vars that the omz bridge
...
function resolveTierAliasToSafeModel(tierAlias) {
  ...
  for (const key of keys) {
    ...
    // omz-internal vars (OMC_SUBAGENT_MODEL, OMC_MODEL_*) are not read by CC, so a [1m]
...
/**
 * Read the `model:` field from an omz agent definition's YAML frontmatter.
...
function readAgentDefinitionModel(subagentType) {
  ...
  const agentType = (typeof subagentType === 'string' ? subagentType : '').replace(/^omz:/, '');
...
// `omz:` namespace, so a caller can hand a skill name to
// Task(subagent_type=...) and receive only Claude Code's generic native
// "Agent type not found". omz owns both registries (agents/*.md and
...
const SKILL_AGENT_NAMESPACE_PREFIXES = ['omz:', 'omc:'];
 ...
 * Namespaced identifiers (omz:X / omc:X) resolve only against
...
/**
 * Claude Code native command names that must not be shadowed by omz skill
 ...
 * (`omz:` / `omc:`) are pinned to the omz plugin namespace and
 * keep the full canonical + shortcut resolution (e.g. `omz:plan`
 * -> `omc-plan`).
 */
...
function evaluateSkillAsAgentCall(toolName, toolInput, directory) {
  ...
  // skills are exposed under the `omz:` namespace (issue #3667
  // review), so the recovery must be unambiguous regardless of the caller's
  // input namespace form.
  const skillIdentifier = `omz:${skill.primary}`;
...
async function generateAgentSpawnMessage(toolInput, stateDir, directory, todoStatus, sessionId) {
  ...
  // Claude Code 2.1.178+ removed TeamCreate/TeamDelete. When omz team state is
...
const SKILL_PROTECTION_MAP = {
  ...
  trace: 'none', hud: 'none', 'omz-doctor': 'none', 'omc-help': 'none',
  ...
  sciomc: 'medium', learner: 'medium', 'omz-setup': 'medium',
  setup: 'medium',        // alias for omz-setup
...
function getSkillProtectionLevel(skillName, rawSkillName) {
  // When rawSkillName is provided, only apply protection to omz-prefixed skills.
  ...
  if (rawSkillName != null && typeof rawSkillName === 'string' &&
      !rawSkillName.toLowerCase().startsWith('omz:')) {
    return 'none';
  }
  const normalized = (skillName || '').toLowerCase().replace(/^omz:/, '');
  return SKILL_PROTECTION_MAP[normalized] || 'none';
}

// Load omz config to check forceInherit setting (issues #1135, #1201)
...
function writeSkillActiveState(stateDir, skillName, sessionId, rawSkillName) {
  ...
  // Nesting guard: when a skill (e.g. omz-setup) invokes a child skill
  ...
  try {
    ...
    if (toolName === 'Skill') {
      ...
      if (skillName) {
        ...
        // Pass rawSkillName to distinguish omz skills from project custom skills (issue #1581)
      ...
      if (isForceInheritEnabled()) {
        ...
        if (!toolModel && toolInput.subagent_type) {
          ...
          if (agentDefModel && !isSubagentSafeModelId(agentDefModel) && !isTierAlias(agentDefModel)
            ...
            const agentType = (toolInput.subagent_type).replace(/^omz:/, '');
            console.log(JSON.stringify({
              continue: true,
              hookSpecificOutput: {
                hookEventName: 'PreToolUse',
