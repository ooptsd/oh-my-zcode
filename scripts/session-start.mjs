...
/**
 * omz Session Start Hook (Node.js)
 ...
 * stable owner process for the interactive session. In installed omz hooks the
...
/**
 * Validate that a candidate cwd is a real omz workspace anchor.
...
function validateCwd(candidate) {
  if (!candidate || typeof candidate !== 'string') {
    process.stderr.write(
      `[omz] session-start: refusing to use cwd '${candidate}' as workspace anchor (no .omc-workspace or .git marker)\n`
    );
    return null;
  }
...
function formatUpdateNoticeForUser(updateInfo, options = {}) {
  ...
  const action = updateInfo?.source === 'marketplace'
    ? 'To update the plugin channel, run: /plugin marketplace update omz && /omz-setup'
    : (options.autoUpgradePrompt === false
      ? 'To update later, run: omz update'
      : 'Run /update to upgrade now, or use /plugin install oh-my-claudecode');
  return `[omz UPDATE AVAILABLE] oh-my-claudecode v${latestVersion} is available (current: v${currentVersion}). ${action}`;
}
...
// Extract omz version from CLAUDE.md content
...
function isManagedPluginCacheRoot(pluginRoot) {
  ...
  // cache path shape still proves it is an omz managed cache version.
...
// version tracked below) survive an omz-only refresh.
...
// as the omz check). Stored alongside the omz fields so the HUD reads one file;
...
// Refresh update caches and return a user-facing omz update notice, if any.
...
async function checkHudInstallation(retryCount = 0) {
  ...
  try {
    if (existsSync(settingsFile)) {
      ...
      // If omz HUD wrapper is configured, ensure at least one plugin cache version is built.
    ...
    if (process.env.CLAUDE_PLUGIN_ROOT) {
      try {
        ...
        if (omcConfig?.silentAutoUpdate) {
          messages.push(`<session-restore>\n\n[omz] silentAutoUpdate is enabled in .omc-config.json but has no effect in plugin mode.\nTo update, use: /plugin marketplace update omz && /omz-setup\nOr run manually: omz update\n\n</session-restore>\n\n---\n`);
        }
      } catch {}
    }
...
[omz] HUD not configured (${hudCheck.reason}). Run /hud setup then restart Claude Code.
</system-reminder>`);
    }
