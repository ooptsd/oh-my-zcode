...
/**
 * omz Keyword Detector Hook (Node.js)
...
// Resolve omz package root: CLAUDE_PLUGIN_ROOT (plugin system) or derive from this script's location
...
function isExplicitRalplanSlashInvocation(prompt) {
  return /^\s*\/(?:omz:)?ralplan(?:\s|$)/i.test(prompt);
}

function isExplicitAskSlashInvocation(prompt) {
  return /^\s*\/(?:omz:)?ask\s+(?:claude|codex|gemini|antigravity|agy|grok|cursor)\b/i.test(prompt);
}

function isRetiredSlashInvocation(prompt) {
  return /^\s*\/(?:omc:|omz:)?(?:ultrawork|ulw|uw|울트라워크|ウルトラワーク|ccg|claude-codex-gemini|씨씨지|シーシージー)(?=\s|$|[?!.,;:])/i.test(prompt);
}
...
const ECHO_CONTINUATION = '(?:\\r?\\n[ \\t]*(?:Task:\\s|When FULLY complete \\(after Architect verification\\)|run\\s+\\/omz:cancel).*)*';
...
function findOfficialRalphLoopNotice(directory) {
  ...
  for (const entry of entries) {
    ...
    if (existsSync(commandFile)) {
      return 'Note: the official Anthropic `ralph-loop` plugin is also installed. `/ralph` runs omz\'s ralph; use `/ralph-loop` for the official plugin.';
    }
  }
...
// Read the omz JSONC config the way src/config/loader.ts does, inlined so the
...
/**
 * Skills the user opted out of via `keywordDetector.disabled` in the omz
...
Preferred invocation: /omz:${skillName}${args ? ` ${args}` : ''}
...
Preferred invocation: /omz:${s.name}${argsText}
${pathStatus}`;
  }).join('\n\n');
