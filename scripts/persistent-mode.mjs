...
/**
 * omz Persistent Mode Hook (Node.js)
 * Minimal continuation enforcer for all omz modes.
...
function isAutopilotRoutingEchoPrompt(promptText) {
  return /^\[MAGIC KEYWORDS?(?: DETECTED)?:\s*AUTOPILOT\s*\]\s*$/i.test(promptText) ||
    /^\/(?:omz:|omc:)?autopilot(?:\s+execute)?\s*$/i.test(promptText);
}
  ...
  try {
    ...
    if (
      ...
      if (sessionMatches) {
        ...
        if (iteration < maxIter) {
          ...
          let reason = `[RALPH LOOP - ITERATION ${iteration + 1}/${maxIter}] Work is NOT done. Continue working.\nWhen FULLY complete (after Architect verification), run /omz:cancel to cleanly exit ralph mode and clean up all state files. If cancel fails, retry with /omz:cancel --force.\n${ralph.state.prompt ? `Task: ${ralph.state.prompt}` : ""}`;
          if (errorGuidance) {
        ...
        if (hardMax > 0 && maxIter >= hardMax) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[RALPH LOOP - HARD LIMIT] Reached hard max iterations (${hardMax}). Mode auto-disabled. Restart with /omz:ralph if needed.`,
            }),
          );
          return;
        }
        ...
        const ralphExtendedReason = `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /omz:cancel (or --force).`;
        console.log(
          JSON.stringify({
            decision: "block",
    ...
    if (
      ...
      if (sessionMatches && !isUltragoalTerminalState(ultragoal.state, omcRoot)) {
        ...
        let reason = `[ULTRAGOAL #${newCount}/${maxReinforcements}] Ultragoal mode is active. Continue the durable goal workflow, keep the matching Claude /goal active, and checkpoint .omc/ultragoal/ledger.jsonl before stopping. When all ultragoal stories are complete and the final quality gate passes, run /omz:cancel to cleanly exit.`;
        ...
        if (phase !== "complete") {
          ...
          if (newCount <= 20) {
            ...
            const cancelGuidance = hasValidSessionId && autopilot.state.session_id === sessionId
              ? " When all phases are complete, run /omz:cancel to cleanly exit and clean up this session's autopilot state files. If cancel fails, retry with /omz:cancel --force."
              : "";
    ...
    if (
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /omz:cancel to cleanly exit and clean up state files. If cancel fails, retry with /omz:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /omz:cancel to cleanly exit and clean up state files. If cancel fails, retry with /omz:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /omz:cancel to cleanly exit and clean up state files. If cancel fails, retry with /omz:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
      ...
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[TEAM - Phase: ${phase}] Team mode active. Continue working. When all team tasks complete, run /omz:cancel to cleanly exit. If cancel fails, retry with /omz:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
    ...
    // Priority 6.5: omz Teams (tmux CLI workers — independent of native team state)
    if (
      ...
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[omz TEAMS - Phase: ${phase}] omz Teams workers active. Continue working. When all workers complete, run /omz:cancel to cleanly exit. If cancel fails, retry with /omz:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
