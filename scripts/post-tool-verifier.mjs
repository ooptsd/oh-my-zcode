...
function buildPreemptiveContextMessage(percentUsed, severity) {
  if (severity === 'critical') {
    return `[omz CRITICAL] Context at ${percentUsed}% (critical threshold: ${getPreemptiveCriticalThreshold()}%). Run /compact now before continuing with more tools or agent fan-out.`;
  }

  return `[omz WARNING] Context at ${percentUsed}% (warning threshold: ${getPreemptiveWarningThreshold()}%). Plan a /compact soon to preserve room for the next large tool output.`;
}
  ...
  for (const trackingFile of candidates) {
    try {
      ...
      if (quietLevel < 2 && running.length > 0) {
        parts.push(`Running: ${running.length} [${running.map(a => a.agent_type.replace('omz:', '')).join(', ')}]`);
      }
